...
<body>
    <div style="width:1840px;border:1px solid black;" >
        ...
        <form action="verif1.php" method="post" >

        
        <div class="form-floating form" >
            <input type="text" class="form-control" placeholder="ide" id="search" style="width:350px;" name="s"/>
        ...
        <table>
            ...
            $result = mysqli_query($conn,"SELECT id,theme,formateur,date_debut,date_fin,nb_heure,statut
            from formations
           
           ");
            while($row = mysqli_fetch_array($result)) {
              
                echo  "<tr><td>".$row['id']."</td><td>".$row['theme']."</td><td>".$row['date_debut']."</td><td>".$row['date_fin']."</td><td>".$row['formateur']."</td><td>".$row['nb_heure']."</td><td>".$row['statut']."</td></tr>";
             }
            ?>
        </table>
    </div>
